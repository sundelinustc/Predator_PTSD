function SDL_SPM12_fMRI_1st_Cov(SDL)
...
for i=1:size(SDL.sbjlist,1) % for each subject
    ...
    matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t = SDL.nslices; % 34
    matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t0 = SDL.nslices/2; 
    ...
    for j = 1:size(SDL.sess,1) % for each session
        ...
        for k = 1:size(SDL.con,1) % for each condition
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess(j).cond(k).duration = fdata(1,2);
