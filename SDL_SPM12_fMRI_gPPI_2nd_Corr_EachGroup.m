function SDL_SPM12_fMRI_gPPI_2nd_Corr_EachGroup(SDL)
    ...
    for j = 1:size(SDL.VOIList,1) % for each VOI
        for k = 1:size(SDL.Cons,1) % for each condition
            for m = 1:size(Pfm,1) % for each performance
                ...
                %% Model specification
                fdir = fullfile(SDL.gPPI_r2nd_dir,['Corr_',SDL.Group{ip,1}],conname,Pfm{m,1},SDL.VOIList{j,1});
                ...
                matlabbatch{1}.spm.stats.con.consess{1}.tcon.name = ['Corr_PPI_',conname,'_',Pfm{m,1},'> 0'];                   % spmT_0001.img
                matlabbatch{1}.spm.stats.con.consess{1}.tcon.convec = [0 1];
                matlabbatch{1}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
                matlabbatch{1}.spm.stats.con.consess{2}.tcon.name = ['Corr_PPI_',conname,'_',Pfm{m,1},'< 0'];                   % spmT_0002.img
