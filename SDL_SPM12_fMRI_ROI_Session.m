function SDL_SPM12_fMRI_ROI_Session(SDL)
...
SDL.fMRI_roi = {
    ...
    'L_Amygdala',          fullfile('Ana','MNI_L_Amygdala_-24_-2_-18_roi.mat');
    'R_Amygdala',          fullfile('Ana','MNI_R_Amygdala_27_-1_-19_roi.mat');
...
%       'R_ParaCentral',       'MNI_R_ParaCentral_7_-33_67_roi.mat';
    ...
    % functional ROIs
    'Pre',                     fullfile('Session','MNI_Pre_-4_-48_8_roi.mat'); % CONT_PTSD,Late-Early_-4_-48_8_roi, from (PTSD-CONT)*(Late-Early)*(Threat-NonThreat), p < 0.005 uncorrected
...
%     'R_vmPFC',                 fullfile('Sinha','MNI_R_vmPFC_roi.mat');
%     'R_vStr',                  fullfile('Sinha','MNI_R_vStr_roi.mat');
%     'L_vlPFC',                 fullfile('Sinha','MNI_L_vlPFC_roi.mat');
%     'L_dmPFC',                 fullfile('Sinha','MNI_L_dmPFC_roi.mat');
%     'L_Ins',                   fullfile('Sinha','MNI_L_Ins_roi.mat');
%     'R_Hipp',                  fullfile('SInha','MNI_R_Hipp_roi.mat');
%     'L_Pre',                   fullfile('Sinha','MNI_L_Pre_roi.mat');
    % These functional masks were from Sinha (2016, PNAS)
    };
...
for j = 1:size(SDL.fMRI_roi,1) % each ROI
    ...
    NewColValue = zeros(size(SDL.sbjlist,1),size(cons,1)*5);% the initial value of the % signal change of the contrast of interest
    for m1 = 1:size(cons,1)
        for m2 = 1:5 % each of 5 sesions
            m = (m1-1)*5 + m2;
            NewColName{m}  = [SDL.fMRI_roi{j,1},'_',cons{m1,1},'_s',num2str(m2)];
        end
    end
    ...
    for i = 1:size(SDL.sbjlist,1) % each subject
        ...
        % Get the mean value of each condition and each session
        NewColValue(i,:) = pct_ev;
...
fn = fullfile(SDL.roi_path,'psc_Session.csv'); % excel file can't be used
