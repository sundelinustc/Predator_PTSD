function SDL_SPM12_gPPI_ROI_corr(SDL)
...
% (2) conditions for extracting gPPI
cons = {
...
SDL.gPPI = SDL.sbjlist(:,{'Subject','Group',...
...
SDL.gPPI % for display purpose
fn = fullfile(SDL.gPPI_r2nd_dir,'gPPI_BetaValue_corr.csv'); % excel file can't be used
writetable(SDL.gPPI,fn);
fprintf('\ngPPI Beta Values saved in: %s\n',fn);


%% T tests: mean(SD) and t(p) values
fprintf('\n\nTable 1. gPPI Beta Values\ngPPIs\tPTSD [mean(SD)]\tCONT [mean(SD)]\t t-value(p-value)\n');
for iCon = 1:size(cons,1) % for each of conditions to be compared, i.e. Threat-none or NonThreat-none
    for iVOI = 1:size(SDL.VOIList,1) % for each seed area
        ...
        for iROI = 1:size(SDL.roi,1) % for each target area
            ...
            % name of the output
            fname = [SDL.VOIList{iVOI,1},'_',...
                cons{iCon,1}{1},'_minus_',cons{iCon,2}{1},'_',...
                SDL.roi{iROI,1}];
            iPTSD = find(ismember(SDL.gPPI.Group,'PTSD')); % find the index of PTSD group members
            iCONT = find(ismember(SDL.gPPI.Group,'CONT')); % find the index of CONT group members
            
            mean_PTSD = mean(SDL.gPPI{iPTSD,fname});std_PTSD = std(SDL.gPPI{iPTSD,fname});
            mean_CONT = mean(SDL.gPPI{iCONT,fname});std_CONT = std(SDL.gPPI{iCONT,fname});
            [H,P,CI,STATS] = ttest2(SDL.gPPI{iPTSD,fname},SDL.gPPI{iCONT,fname}); % 2 sample t test
            plist = [plist P];
...
fprintf('\n\nTable 2. Correlation between gPPI beta values and behavioral performance.\nBehav\tgPPIs\tPTSD [R(p)]\tCONT [R(p)]\t between-group t-value(p-value)\n');
for iCon = 1:size(SDL.con,1) % for each of conditions to do correlation analyses
    for iVOI = 1:size(SDL.VOIList,1) % for each seed area
        ...
        for iROI = 1:size(SDL.roi,1) % for each target area
            ...
            fname2 = [SDL.VOIList{iVOI,1},'_',SDL.con{iCon,2},'_',SDL.roi{iROI,1}];
            fname3 = SDL.con{iCon,3};
            iPTSD = find(ismember(SDL.gPPI.Group,'PTSD')); % find the index of PTSD group members
            iCONT = find(ismember(SDL.gPPI.Group,'CONT')); % find the index of CONT group members
            
            [R,p] = partialcorr(SDL.gPPI{iPTSD,{fname1,fname2,fname3{:}}}); R_PTSD=R(1,2); p_PTSD=p(1,2); % correlation between col 1&2 is at matrix(1,2)
            [R,p] = partialcorr(SDL.gPPI{iCONT,{fname1,fname2,fname3{:}}}); R_CONT=R(1,2); p_CONT=p(1,2); % correlation between col 1&2 is at matrix(1,2)
